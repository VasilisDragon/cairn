...
export async function run(bot, params = {}, ctx = {}) {
  ...
  while (attempts < maxAttempts && catches < target.catches && Date.now() < deadlineAtMs) {
    ...
    try {
      ...
      if (isTransientCastFailure(err) && attempts < maxAttempts && Date.now() < deadlineAtMs) {
        continue;
      }
...
function fishOnce(bot, opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    timeout = setTimeout(() => {
      fail(new Error(`fishing attempt timed out after ${attemptTimeoutMs}ms; bobber=${bobber ? 'spawned' : 'not-spawned'}`));
    }, attemptTimeoutMs);
