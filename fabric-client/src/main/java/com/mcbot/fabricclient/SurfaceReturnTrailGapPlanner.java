...
final class SurfaceReturnTrailGapPlanner {
    ...
    private SurfaceReturnTrailGapPlanner() {
    }
    ...
    static Result plan(
        ...
        if (
            horizontalGap > MAX_HORIZONTAL_MANHATTAN
        ...
        while (!open.isEmpty()) {
            ...
            if (
                node.costFromStart() != bestCost.getOrDefault(cell, Integer.MAX_VALUE)
                    || !closed.add(cell)
            ) {
                continue;
            }
